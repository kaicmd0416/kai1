...
class portfolio_tracking:
    def __init__(self,product_name,available_date):
        ...
        self.df_stock,self.df_bond,self.df_cbond,self.df_future,self.df_option=prod.position_processing()
        df_info=prod.product_info_withdraw()
        self.stock_money = df_info['股票市值'].tolist()[0]
        self.asset_value = df_info['资产净值'].tolist()[0]
        self.asset_value_yes=prod.asset_yes_withdraw()
    ...
    def option_analysis(self,yes):
        ...
        merged_df = merged_df[
            ['科目名称', '方向', '数量', '前结算价', '现价', 'Delta', '中价隐含波动率', 'future', 'future_price',
             'ratio','市价']]
    ...
    def FOC_main(self,cb_mkt_value,cb_profit):
        ...
        mkt_value_total = df_final['market_value'].sum()+cb_mkt_value
        profit_total = df_final['daily_profit'].sum()+cb_profit
        df_info['info_name']=['转货权总市值','转货权总盈亏','期权市值','期货市值','可转债市值','期权盈亏','期货盈亏','可转债盈亏','国债市值','国债盈亏']
        df_info['money']=[mkt_value_total,profit_total,mkt_value_option,mkt_value_future,cb_mkt_value,profit_option,profit_future,cb_profit,b_mkt,b_profit]
        return df_info,df_final

    #产品指标处理部分
    def portfolio_info_processing(self,df_info):
        df_porinfo=pd.DataFrame()
        mkt_value_total=df_info[df_info['info_name']=='转货权总市值']['money'].tolist()[0]
        ...
        df_porinfo['info_name']=['杠杆率','股票占比','期货占比','期权占比','可转债','股票市值','国债市值','资产净值']
        df_porinfo['money']=[leverage_ratio,ratio_stock,ratio_future,ratio_option,ratio_cb,self.stock_money,mkt_value_b,self.asset_value]
        return df_porinfo
    #仓位对比
    ...
    def product_return_split_zhizeng(self,df_info,stock_profit,stock_money,df_porinfo,cb_profit,cb_mkt_value_yes):
        ...
        sum_p=abs(option_yes)+abs(future_yes)+abs(cb_mkt_value_yes)+stock_money
        sum_p2=option_proportion+future_proportion+stock_proportion+cb_proportion
        df_pro=pd.DataFrame()
        df_pro['product_split']=['产品','股票','期权','期货','转债','国债']
        error_pro=product_profit-stock_profit-option_pro-future_pro-cb_profit-bond_pro
        error_return=error_pro/self.asset_value_yes
        df_pro['盈亏']=[product_profit,stock_profit,option_pro,future_pro,cb_profit,bond_pro]
        df_pro['收益率(本身)bp']=[product_return,stock_return,option_return,future_return,cb_return,bond_return]
    ...
    def product_return_split_zhongxing(self,df_info,stock_profit,stock_money,df_porinfo,cb_profit,cb_mkt_value_yes):
        ...
        error_pro=product_profit-stock_profit-option_pro-future_pro-cb_profit-bond_pro
        error_return=error_pro/self.asset_value_yes
        df_pro['昨日金额']=[self.asset_value_yes,stock_money,option_yes,future_yes,cb_mkt_value_yes,None]
        df_pro['盈亏']=[product_profit,stock_profit,option_pro,future_pro,cb_profit,bond_pro]
        df_pro['收益率(本身)bp']=[product_return,stock_return,option_return,future_return,cb_return,bond_return]
        df_pro['超额(本身)bp']=df_pro['收益率(本身)bp']-float(self.index_return)
        ...
        df_pro['仓位占比(昨日)'] = [0, stock_proportion, option_proportion, future_proportion,cb_proportion,0]
