...
class data_prepared:
    ...
    def realtime_data_withdraw(self,yes):
        ...
        columns=realtime_data_index.columns.tolist()
        if '000510.SH' in columns:
...
class product_data:
    ...
    def position_withdraw(self):
        ...
        return df_stock,df_bond,df_cbond,df_future,df_option
    ...
    def position_processing(self):
        df_stock, df_bond,df_cbond, df_future, df_option=self.position_withdraw()
        df_option['科目名称']=df_option['科目名称'].apply(lambda x: str(x)[:-4])
        df_bond=df_bond[['代码','数量','市价','市值']]
        ...
        df_bond_yes=self.yes_position_withdraw()
